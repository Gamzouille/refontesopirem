...
def build_ping_steps(self, source_item, destination_item, path):
    ...
    if source_pc.arp_table.get_mac(destination_pc.ip) != destination_pc.mac:
        ...
        arp_trame = Trame(source_ip=source_pc.ip, dest_ip=destination_pc.ip,
                          source_mac=source_pc.mac, dest_mac="FF:FF:FF:FF:FF:FF", type_trame="ARP")
        steps.extend(
            {
                "segments": [{"cable": cable, "from_item": from_item, "to_item": to_item}
        ...
        arp_reply_trame = Trame(source_ip=destination_pc.ip, dest_ip=source_pc.ip,
                                source_mac=destination_pc.mac, dest_mac=source_pc.mac, type_trame="ARP-REPLY")
        destination_pc.trames_envoyees.append(arp_reply_trame)
        steps.extend(
            {
                "segments": [{"cable": cable, "from_item": from_item, "to_item": to_item}],
                "color": QColor("#3498db"),
                "phase": "ArpReply",
                "trame": arp_reply_trame,
            }
    ...
    icmp_trame = Trame(source_ip=source_pc.ip, dest_ip=destination_pc.ip,
                       source_mac=source_pc.mac, dest_mac=destination_pc.mac, type_trame="ICMP")
    source_pc.trames_envoyees.append(icmp_trame)
    steps.extend(
        {
            "segments": [{"cable": cable, "from_item": from_item, "to_item": to_item}],
            "color": QColor("#20b15a"),
            "phase": "IcmpEchoRequest",
            "trame": icmp_trame,
        }
    ...
    icmp_reply_trame = Trame(source_ip=destination_pc.ip, dest_ip=source_pc.ip,
                             source_mac=destination_pc.mac, dest_mac=source_pc.mac, type_trame="ICMP-REPLY")
    destination_pc.trames_envoyees.append(icmp_reply_trame)
    steps.extend(
        {
            "segments": [{"cable": cable, "from_item": from_item, "to_item": to_item}],
            "color": QColor("#2ecc71"),
            "phase": "IcmpEchoResponse",
            "trame": icmp_reply_trame,
        }
        for cable, from_item, to_item in reverse_path_result
    )
    learn_switch_mac_along_path(self, destination_pc.mac, reverse_path_result)
...
def launch_ping(self, source_item, destination_item):
    ...
    # --- Overlay dès le départ ---
    t = first_step["trame"]
    self.show_trame_overlay(
        source_mac=t.source_mac,
        dest_mac=t.dest_mac,
        source_ip=t.source_ip,
        dest_ip=t.dest_ip,
        type_trame=first_step["phase"],
    )
    ...
    self.ping_animation_timer.start(75)
...
def advance_ping_animation(self):
    ...
    if self.current_ping_step >= len(self.current_ping_steps):
        ...
        QTimer.singleShot(1500, lambda: self.hide_trame_overlay())
        self.ping_animation_timer.stop()
        return
